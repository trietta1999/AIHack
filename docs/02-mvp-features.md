...
| --- | --- | --- | --- |
| F1 | Natural-language Q&A on Vietnam travel (Vietnamese + English) | ✅ Implemented | `search_travel_knowledge` tool + Azure GPT-4o-mini |
| F2 | RAG over markdown knowledge base (6 files / 26 documents / 31 chunks) | ✅ Implemented | `travel_advisor/ingestion.py` + `travel_advisor/retriever.py` (FAISS IndexFlatIP) |
...
| F13 | **One-command Docker deployment** | ✅ Implemented | [Dockerfile](../Dockerfile) + [docker-compose.yml](../docker-compose.yml) |
| F14 | **Makefile + smoke-test** for graders | ✅ Implemented | [Makefile](../Makefile) + [scripts/smoke_test.sh](../scripts/smoke_test.sh) |
| F15 | Automated screenshot capture (5 demo screens) | ✅ Implemented | [scripts/capture_screenshots.py](../scripts/capture_screenshots.py) |
| F16 | Slide export pipeline (Markdown → PDF + PPTX) | ✅ Implemented | [scripts/export_slides.sh](../scripts/export_slides.sh) |
| F17 | Offline test suite (29 tests, no API key needed) | ✅ Implemented | `tests/` with `HashingEmbedder` + `FakeToolCallingChatModel` |
