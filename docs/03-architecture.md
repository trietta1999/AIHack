...
flowchart LR
    ...
    db[(SQLite<br/>data/bookings.sqlite)]
...
```mermaid
flowchart LR
    md[data/knowledge_base/*.md] -->|YAML frontmatter parse| ing[ingestion.chunks_from_dir]
    ing -->|paragraph packing<br/>max_chars=1200| chunks[Chunk objects]
    chunks -->|embed| azemb[Azure embeddings<br/>text-embedding-3-small]
    azemb -->|L2-normalised float32| vecs[NumPy matrix N x 1536]
...
sequenceDiagram
    ...
    participant SQL as query_tour_inventory
    participant BUD as estimate_trip_budget

    U->>UI: Question
    UI->>A: HumanMessage + thread_id
    ...
    LLM-->>A: tool_call(estimate_trip_budget, args)<br/>or final AIMessage
    Note over A,LLM: Loop until LLM emits a final AIMessage (no more tool_calls).
    LLM-->>A: final AIMessage
    A-->>UI: TurnResult(answer, trace)
...
| **Streamlit (not React)** | Single-process Python deployment matches the hackathon's "localhost" deployment story. ~150 LOC for a working chat UI with citations. |
