...
| --- | --- | --- |
| **Automated** | If you have a valid `AZURE_OPENAI_API_KEY` and want reproducible captures. | `make screenshots` (installs Playwright + Chromium, opens the app headlessly, walks through 5 scenarios, saves PNGs here). |
...
| --- | --- |
| `01-initial-state.png` | Empty chat with the sidebar visible (API key field, Boot button, sample-query buttons). |
| `02-rag-vietnamese.png` | Reply to S01 ("Mình có 3 ngày ở Hà Nội...") — assistant message in Vietnamese with `(doc_id)` citations. |
| `03-tool-call-expanded.png` | Same turn as #02 but with the **Tool calls** expander opened so the `search_travel_knowledge` args + raw observation are visible. |
| `04-multi-tool-budget.png` | Reply to S05 or S06 — assistant message containing both an itinerary/snippets AND a budget breakdown, with tool-trace expander showing 2+ tool calls. |
